"""Run Basis ADX 1D with OLD fixed 0.2% TP and save HTML."""
...
class BasisAdxFixedTP(BasisAdxStrategy):
...
bt = Backtest(df, BasisAdxFixedTP, cash=100_000, commission=0.001, finalize_trades=True)
stats = bt.run()

print(f"\nIBM 1D — Basis ADX (fixed 0.2% TP)")
print(f"  Return   : {stats['Return [%]']:.2f}%")
print(f"  B&H      : {stats['Buy & Hold Return [%]']:.2f}%")
print(f"  Sharpe   : {stats['Sharpe Ratio']:.2f}")
print(f"  Max DD   : {stats['Max. Drawdown [%]']:.2f}%")
print(f"  Trades   : {stats['# Trades']}")
print(f"  Win Rate : {stats.get('Win Rate [%]', 0):.1f}%")

report_path = BASE/"reports"/"IBM_basis_adx_IBM_1d.html"
