"""
Backtest Mean Reversion ADX — TSLA Daily, last 5 years.
"""
...
print("=" * 60)
print("  Mean Reversion ADX — TSLA Daily (5 tahun)")
print("=" * 60)
...
r = f"${data['Low'].min():.2f} — ${data['High'].max():.2f}"
print(f"  Range: {r}")
...
for adx_val in [20, 25, 30]:
    ...
    bt = Backtest(data, Override, cash=100_000, commission=0.001, finalize_trades=True)
    stats = bt.run()
...
bt = Backtest(data, Best, cash=100_000, commission=0.001, finalize_trades=True)
stats = bt.run()
...
bt.plot(filename=str(out / 'TSLA_mean_rev_adx.html'), open_browser=False)
print(f"  HTML: {out / 'TSLA_mean_rev_adx.html'}")
