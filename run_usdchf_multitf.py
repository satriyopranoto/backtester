"""
USD/CHF 1h — Single TF vs Multi TF + CFD (0.05 lot)
"""
import numpy as np, pandas as pd, yfinance as yf, warnings
warnings.filterwarnings('ignore')

print("Loading USD/CHF 1h & Daily...")
t = 'USDCHF=X'
...
MODAL = 1000.0; LEVERAGE = 100; LOT = 0.05; STOP_OUT = 30; CONTRACT = 100000 * LOT
...
print(f"\n\n{'='*50}")
print(f"  PERBANDINGAN USD/CHF")
print(f"{'='*50}")
print(f"  {'Metrik':18s} {'Single TF':15s} {'Multi TF':15s}")
print('  ' + '-'*48)
for m in ['Return','CAGR','Sharpe','Ulcer','Max DD','Trades','WR','MC']:
    k = m.lower().replace(' ','_')
    s = results[0].get(k, 0)
