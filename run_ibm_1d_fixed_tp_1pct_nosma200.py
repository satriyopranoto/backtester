"""Run Basis ADX 1D with fixed 1.0% TP — NO SMA 200 filter."""
...
class BasisAdxNoSma200(BasisAdxStrategy):
    """Basis ADX fixed 1.0% TP — tanpa filter SMA 200 uptrend."""
    tp_min_profit_pct = 1.0

    def init(self):
        # Panggil init parent — tapi kita ignore sma200 di entry nanti
        super().init()
    ...
    def next(self):
        ...
        # sma200 masih dihitung parent tapi TIDAK dipake buat entry
        ...
        # ── ENTRY ── (NO sma200 filter!)
        if (low > sl and close > basis and not is_nan and adx > 20.0
...
bt = Backtest(df, BasisAdxNoSma200, cash=100_000, commission=0.001, finalize_trades=True)
stats = bt.run()

print(f"\n{'='*60}")
print(f"  IBM 1D — Basis ADX (fixed 1.0% TP) — NO SMA 200")
print(f"{'='*60}")
...
report_path = BASE/"reports"/"IBM_basis_adx_fixed_1pct_nosma200_1d.html"
